define([
...
], function(module, Handlebars, Chaplin) {
  'use strict';
  ...
  var View = Chaplin.View.extend({
    ...
    preParseTemplate: function(){
      if(typeof this.template !== 'string') return true
      if(!this.uri) {
        throw new TypeError("View#preParseTemplate: You should specify 'this.uri' in View that shows View's location");
        return false
      } 

      this.moduleUri = this.uri.replace(/\/views\/.*/g, "")
      this.template  = this.template.replace(/\{\{\%moduleUri\}\}/g, this.moduleUri)
      return true
    },
